...
int wzStanceDetector::createAlphabet(const vector<Instance>& vecInsts) {
	...
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		int id = m_driver._modelparams.labelAlpha.from_string(label);
...
void wzStanceDetector::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_driver._modelparams.targetAlpha.initial(m_target_stats, m_options.targetCutOff);
		...
		if (devNum > 0) {
			...
			if (devAcc > max) {
				...
				if (step == 10) {
					...
					std::cout << "the best dev iter is: " << best_iter << std::endl;
					exit(0);
				}
			}
...
void wzStanceDetector::loadModelFile(const string& inputModelFile) {
...
void wzStanceDetector::writeModelFile(const string& outputModelFile) {
