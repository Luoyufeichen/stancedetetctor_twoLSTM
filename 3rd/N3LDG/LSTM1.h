...
public:
	inline void forward(Graph *cg, const vector<PNode>& x, LSTM1Params *Params, PAddNode *&targetcell, bool bTarget) {
		if (x.size() == 0) {
		...
		if (_left2right) {
			return left2right_forward(cg, x, targetcell, Params, bTarget);
		}
		else {
			return right2left_forward(cg, x, targetcell, Params, bTarget);
		}
	}

protected:
	inline void left2right_forward(Graph *cg, const vector<PNode>& x, PAddNode *&targetcell, LSTM1Params *param, bool bTarget) {
		for (int idx = 0; idx < _nSize; idx++) {
			...
			if (idx == 0) {
				if (bTarget) {
					...
					_halfcells[idx].forward(cg, &_bucket, x[idx]);
					_inputfilters[idx].forward(cg, &_halfcells[idx], &_inputgates[idx]);
					//_forgetfilters[idx].forward(cg, &targetcell, &_forgetgates[idx]);
					_cells[idx].forward(cg, &_inputfilters[idx], targetcell);
					_halfhiddens[idx].forward(cg, &_cells[idx]);
					_outputgates[idx].forward(cg, &_bucket, x[idx]);
					_hiddens[idx].forward(cg, &_halfhiddens[idx], &_outputgates[idx]);
				}
	...
	inline void right2left_forward(Graph *cg, const vector<PNode>& x, PAddNode *&targetcell, LSTM1Params *Params, bool bTarget) {
		...
		for (int idx = _nSize - 1; idx >= 0; idx--) {
			...
			if (idx == _nSize - 1) {
				if (bTarget) {
					_bucket.forward(cg, 0);
					_bucket.node_name = (boost::format("bucket target %1%") % idx).str();

					_inputgates[idx].forward(cg, &_bucket, x[idx]);
					_inputgates[idx].node_name = (boost::format("_inputgates target %1%") % idx).str();
